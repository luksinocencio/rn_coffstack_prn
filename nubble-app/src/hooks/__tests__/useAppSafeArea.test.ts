...
describe('useAppSafeArea', () => {
  it('when the safe area is less than minimum requiriment, it returns the minimum requirement', async () => {
    ...
    const { result } = await renderHook(() => useAppSafeArea())

    expect(result.current.top).toEqual(theme.spacing.s20)
    expect(result.current.bottom).toEqual(theme.spacing.s20)
  })

  it('when the safe area is greather than minimum requiriment, it returns the safe area', async () => {
    mockedUseSafeAreaInsents.mockImplementationOnce(
      () =>
        ({
          top: 40,
