...
describe('integration: SearchScreen', () => {
  it('Search Flow', async () => {
    ...
    await renderScreen(<AppStack initialRouteName="SearchScreen" />)

    // 2) Find the search input and type user name
    const inputText = screen.getByPlaceholderText(/digite para buscar/i)
    await fireEvent.changeText(inputText, 'mar')
    // O `act` precisa ser aguardado: além de disparar o debounce, os timers
    // liberam a request do react-query, cujo resultado chega numa microtask.
    await act(async () => {
      jest.runAllTimers()
    })
    ...
    await fireEvent.changeText(inputTextAfterBack, '')
    await act(async () => {
      jest.runAllTimers()
    })
    ...
    await fireEvent.press(trashIcon)

    // 12) Make sure the user1 is not in the search history anymore.
    // A remoção passa pelo storage (assíncrono), então o re-render só acontece
    // depois que a promise resolve — daí o `waitFor` em vez de checar direto.
    await waitFor(() => {
      expect(screen.queryByText(userMocked.user1.username)).toBeFalsy()
    })
  })
})
