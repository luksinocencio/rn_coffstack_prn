...
import { CitiesGroupedByCategory, CityToggleFavoriteParams, ICityRepo } from '@/domain/city/ICityRepo'
...
const CITY_PREVIEW_FIELD = 'id,name,country,cover_image,favorite_cities!left(user_id)'
...
async function toggleFavorite(params: CityToggleFavoriteParams): Promise<void> {
  ...
  if (params.isFavorite) {
    await supabase.from('favorite_cities').delete().eq('user_id', user.id).eq('city_id', params.cityId)
  } else {
    await supabase.from('favorite_cities').insert({ city_id: params.cityId, user_id: user.id })
  }
}
...
async function findGroupedByCategory(): Promise<CitiesGroupedByCategory[]> {
  ...
  return data.map(item => ({
    ...
    cities: item.city_categories.map(data => supabaseAdapter.toCityPreview(data.cities)),
  }))
}
